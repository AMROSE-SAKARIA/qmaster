...
def register():
    ...
    otps[username] = {"otp": otp, "expires": datetime.now() + timedelta(minutes=10)}
    try:
...
def upload_content():
    ...
    # Check input type (text or PDF)
    input_type = request.form.get('inputType')  # 'text' or 'pdf'
    ...
    try:
        # Determine the content to use for question generation based on input_type
        ...
        # Generate both MCQs and descriptive questions from the same content
...
def create_test():
    ...
    selected_mcqs = data.get('selectedMCQs', [])
    selected_descriptive = data.get('selectedDescriptive', [])
    ...
    logger.info(f"Selected MCQs: {len(selected_mcqs)}, Selected Descriptive: {len(selected_descriptive)}")
    logger.info(f"Selected MCQ IDs: {[mcq['id'] for mcq in selected_mcqs]}")
    logger.info(f"Selected Descriptive IDs: {[desc['id'] for desc in selected_descriptive]}")
    if not all([token, selected_mcqs, selected_descriptive, desired_mcqs, desired_descriptive]):
        return jsonify({"error": "Missing required fields"}), 400
    mcqs = list(questions.find({"token": token, "type": "mcq"}))
    descriptive = list(questions.find({"token": token, "type": "descriptive"}))
    if desired_mcqs > len(mcqs):
        logger.warning(f"Adjusting desiredMCQs from {desired_mcqs} to {len(mcqs)} due to insufficient questions")
        desired_mcqs = len(mcqs)
    if desired_descriptive > len(descriptive):
        logger.warning(f"Adjusting desiredDescriptive from {desired_descriptive} to {len(descriptive)} due to insufficient questions")
        desired_descriptive = len(descriptive)
    selected_mcqs = selected_mcqs[:desired_mcqs]
    selected_descriptive = selected_descriptive[:desired_descriptive]
    if len(selected_mcqs) < desired_mcqs or len(selected_descriptive) < desired_descriptive:
        error_msg = f"Insufficient selected questions. Expected {desired_mcqs} MCQs and {desired_descriptive} Descriptive, but got {len(selected_mcqs)} MCQs and {len(selected_descriptive)} Descriptive."
        logger.error(error_msg)
        return jsonify({"error": error_msg}), 400
    mcq_ids = [str(doc['_id']) for doc in mcqs]
    descriptive_ids = [str(doc['_id']) for doc in descriptive]
    invalid_mcqs = [mcq for mcq in selected_mcqs if mcq['id'] not in mcq_ids]
    invalid_descriptive = [desc for desc in selected_descriptive if desc['id'] not in descriptive_ids]
    if invalid_mcqs or invalid_descriptive:
        logger.error(f"Invalid MCQ IDs: {[mcq['id'] for mcq in invalid_mcqs]}")
        logger.error(f"Invalid Descriptive IDs: {[desc['id'] for desc in invalid_descriptive]}")
        return jsonify({"error": f"Invalid question IDs - Invalid MCQs: {[mcq['id'] for mcq in invalid_mcqs]}, Invalid Descriptive: {[desc['id'] for desc in invalid_descriptive]}"}), 400
    test_config = {
        "token": token,
        "desiredMCQs": desired_mcqs,
        "desiredDescriptive": desired_descriptive,
        "selectedMCQs": selected_mcqs,
        "selectedDescriptive": selected_descriptive,
        "createdAt": datetime.now()
    }
    ...
    return jsonify({"testToken": token, "message": f"Test created with {desired_mcqs} MCQs and {desired_descriptive} descriptive questions"}), 201
...
def join_test():
    ...
    mcqs = list(questions.aggregate([
        {"$match": {"token": token, "type": "mcq"}},
        {"$sample": {"size": desired_mcqs}}
    ]))
    descriptive = list(questions.aggregate([
        {"$match": {"token": token, "type": "descriptive"}},
        {"$sample": {"size": desired_descriptive}}
    ]))
    valid_mcqs = []
    for mcq in mcqs:
        if (mcq.get('question') and not mcq['question'].startswith("Generate") and len(mcq['question']) > 10 and
    ...
    valid_descriptive = []
    for desc in descriptive:
        if desc.get('question') and not desc['question'].startswith("Generate") and len(desc['question']) > 10:
            valid_descriptive.append(desc)
    if len(valid_mcqs) < desired_mcqs or len(valid_descriptive) < desired_descriptive:
        return jsonify({"error": "Not enough valid questions available"}), 400
    for item in valid_mcqs + valid_descriptive:
        if '_id' in item:
            item['_id'] = str(item['_id'])
    return jsonify({"mcqs": valid_mcqs, "descriptive": valid_descriptive, "token": token}), 200
...
def leaderboard(token):
    ...
    for item in submissions_list:
        if '_id' in item:
            item['_id'] = str(item['_id'])
    return jsonify({"submissions": submissions_list, "classAverage": round(class_average, 2), "totalQuestions": total_questions}), 200
